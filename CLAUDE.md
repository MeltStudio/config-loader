...
`@meltstudio/config-loader` — a TypeScript library providing a fluent, type-safe API for loading configuration from YAML, JSON, and TOML files, `.env` files, environment variables, and CLI arguments into a single typed object. Priority order: CLI > process.env > `.env` files > Config files > Defaults.
...
- TypeScript 5, Node 20+, tsup for bundling
- Jest + ts-jest for testing
- ESLint (extends `@meltstudio/eslint-config/node-ts`), Prettier
- Runtime deps: `js-yaml`, `smol-toml`, `commander`, `tslib`
